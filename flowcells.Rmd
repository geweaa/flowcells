...
adiv %>%
  ...
  scale_y_discrete(labels = 
                     c(expression(atop(Biofilm~TM-448.4, paste(~italic("n")~" = 6"))),
                       expression(atop(Planktonic~TM-448.4, paste(~italic("n")~" = 12"))),
                       expression(atop(Biofilm~MM-171.3, paste(~italic("n")~" = 6"))),
                       expression(atop(Planktonic~MM-171.3, paste(~italic("n")~" = 12")))
                       )
                   ) +
...
```{r export Fig. 2}
ggsave("figures/fig_2.pdf", width = 16, height = 10, units = "cm")
ggsave("figures/fig_2.png", width = 16, height = 10, units = "cm")
```
...
ggsave("figures/fig_s2.pdf", height = 10, width = 12, units = "cm")
ggsave("figures/fig_s2.png", height = 10, width = 12, units = "cm")
```
...
```{r export Fig. 3}
ggsave("figures/fig_3.pdf", height = 10, width = 18, units = "cm")
ggsave("figures/fig_3.png", height = 10, width = 18, units = "cm")
```
...
## Fig. 4a: NMDS on Bray-Curtis dissimilarities (RNA)
...
nmds.scores %>%
  ...
  geom_hline(yintercept = 0, linetype = 'dotted') +
  theme_tidy() +
  theme(legend.spacing.x = unit(0.0, "mm")) -> a
```
...
```{r Fig. s3 barplot biofilm order level eggnog}
...
```{r Fig. s3 export barplot eggnog}
...
ggsave("figures/fig_s4.pdf", height = 10, width = 10, units = "cm")
ggsave("figures/fig_s4.png", height = 10, width = 10, units = "cm")
```
...
ggsave('figures/fig_s5.pdf', height = 10, width = 16, units = 'cm')
ggsave('figures/fig_s5.png', height = 10, width = 16, units = 'cm')
```

```{r}
diffexp %>%
  # Select significant DE genes
  filter(diffexp != 0 & eggnog %in% i) %>%
  mutate(logFC = logFC * -1) %>%
  # Call plot
  ggplot(aes(x = logFC, fct_relevel(eggnog, i))) +
  labs(x = 'Log fold-change', y = '') + 
  geom_violin() +
  facet_wrap(~ aquifer, scales = "free_x") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 25)) +
  theme_tidy(ratio = 3.0) +
  theme(panel.border = element_blank(),
        strip.background = element_rect(size = 0,fill = "#f0f0f0"),
        axis.ticks.y = element_blank(),
        strip.text = element_text(face = "bold"),
...
ggsave('figures/fig_s6.pdf', height = 10, width = 10, units = 'cm')
ggsave('figures/fig_s6.png', height = 10, width = 10, units = 'cm')
```
```
