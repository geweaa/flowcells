...
```{r export Fig. 3}
ggsave("figures/fig_3.pdf", width = 16, height = 10, units = "cm")
ggsave("figures/fig_3.png", width = 16, height = 10, units = "cm")
```
...
cols <- c("Other" = "#a6cee3", "Unidentified" = "#d9d9d9",
          "Ralstonia" = "#1f78b4","Immundisolibacter" = "#045a8d",
...
barplot(taxlevel = "genus", n = 10) %>%
  ...
  ggplot(aes(x = fct_relevel(env, c('TM-448.4 75d', 'TM-448.4 40d', 'TM-448.4 20d',
             ...
             fill = fct_relevel(topphylum, c("Other","Unidentified",
                                             "Ralstonia","Immundisolibacter","Sulfurimonas",
...
# Oxidation
prokka %>% 
  filter(EC_number %in% c("2.7.7.4","2.8.5.2") | 
           COG %in% c("COG3302","COG0243","COG0437","COG0446")) %>% 
  select(geneid, product) %>% 
...
sulfur %>%
  ...
  scale_x_continuous(expand = c(0.004,0), n.breaks = 4,
                     limits = c(0,30000),
                     labels = c("0","10","20","30")) +
  theme_clean(ratio = 1.8, fontsize = 8) +
...
```
```{r export redox}
ggsave('figures/fig_s5.pdf', height = 10, width = 8.5, units = 'cm')
ggsave('figures/fig_s5.png', height = 10, width = 8.5, units = 'cm')
```
