...
asvs %>%
  ...
  mutate(env = paste(designation,age)) %>%
...
adiv %>%
  ...
  theme_clean() +
  theme(plot.tag.position = c(.2,.99))  -> a
```
...
pca.samples %>%
  ...
  theme_tidy() +
  theme(legend.spacing.x = unit(0.0, "mm")) -> b
```
...
```{r export Fig. 1}
ggsave("figures/fig_1.pdf", width = 18, height = 10, units = "cm")
ggsave("figures/fig_1.png", width = 18, height = 10, units = "cm")
```
...
pca.samples %>%
  ...
  geom_hline(yintercept = 0, linetype = 'dotted') +
  theme_tidy() +
  theme(legend.spacing.x = unit(-0.2, "mm")) -> a
```
...
seqcpm %>%
  ...
  guides(fill = guide_legend(nrow = 4)) +
  theme(axis.text.y = element_text(size = 6))-> b
```
