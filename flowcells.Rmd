...
Transform absolute counts to counts per million (cpm)
...
diffexp %>%
  ...
  theme(aspect.ratio = 3.0, 
        ...
        panel.border = element_blank(), strip
...
### Pie chart Thiobacillus ###


```{r}
rnatab %>%
  inner_join(ncbi, by = "geneid") %>%
  select(sample, count, geneid) %>%
  spread(sample, count, fill = 0) %>%
  column_to_rownames('geneid') %>%
  edgeR::DGEList(group = smd_metat$treat) %>% edgeR::cpm() %>%
  as.data.frame() %>%
...
eggnogs %>%
  left_join(t %>% transmute(eggnog, topnog = eggnog), by = "eggnog") %>%
  replace_na(list("topnog" = "Other")) -> taxref
```


```{r barplot eggnog thiobacillus}
smd_metat %>% mutate(ref = paste(nature, designation, sep = '\n')) -> smd_metat

seqcpm %>%
  inner_join(ncbi, by = "geneid") %>% filter(genus == "Thiobacillus") %>%
  left_join(taxref, by = "geneid") %>%
  replace_na(list("topnog" = "Unassigned")) %>%
  # Combine water type and origin
  inner_join(smd_metat, by = 'sample') %>%
  # Summarize in order to have the sum for each category and topphylum
  group_by(ref, topnog) %>% summarise(cpm = sum(cpm), .groups = "drop_last") %>%
  #mutate(cpm = cpm / sum(cpm)) %>%
  # Call plot
  ggplot(aes(x = fct_relevel(ref, c('Biofilm\nTM-448.4','Planktonic\nTM-448.4')), 
             y = cpm, 
             fill = fct_relevel(topnog, c("Unassigned", 'Other')))) +
  labs(x = '', y = 'Counts per million') + 
  scale_y_continuous(labels = c("1.0e+6", "7.5e+5", "5.0e+5", "2.5e+5", "0.0"), trans = 'reverse') +
  geom_col(width = 0.8) + 
  scale_fill_brewer(palette = "Paired") +
  coord_flip() + theme_barplot() + guides(fill = guide_legend(nrow = 6)) + 
  theme(aspect.ratio = 0.6, legend.key.size = unit(4, 'mm'))
```


```{r export barplot mRNA eggnog}
ggsave("figures/mrna_eggnog.pdf", height = 10, width = 16, units = "cm")
```
