...
adiv %>% group_by(env,treat,age) %>% summarise(meandiv = mean(shannon), .groups = "drop") -> adiv
...
adiv %>%
  ...
  scale_x_continuous(limits = c(1,6), expand = c(0.004,0)) +
...
barplot(taxlevel = "order", n = 11) %>%
  ...
  annotate(geom = "segment", x = .55, xend = 6.45, y = -.01, yend = -.01) +
  annotate(geom = "segment", x = 6.55, xend = 8.45, y = -.01, yend = -.01) +
  annotate('text', x = 3.5, y = -0.04, size = 6 / .pt, label = "Biofilm", angle = 90, hjust = 0.5) +
  annotate('text', x = 7.5, y = -0.04, size = 6 / .pt, label = "Planktonic", angle = 90, hjust = 0.5) +
  coord_flip() + 
  scale_y_continuous(trans = 'reverse', labels = c('100','75','50','25','0')) + 
  theme_barplot(ratio = 1.5) + 
...
seqtab %>%
  ...
  geom_venn(
    ...
    stroke_size = .5) + 
  annotate(geom = "text", x = -0.75, y = -.2, label = "(84%)", size = 6/.pt) +
  annotate(geom = "text", x = .8, y = -.2, label = "(80%)", size = 6/.pt) +
  coord_fixed(expand = FALSE) +
  theme_void() + 
  theme(aspect.ratio = 1, plot.tag.position = c(.1,1.02),
        plot.margin = margin(0,0,0,0)) #-> c
```
...
a + b + c + 
  ...
  plot_layout(nrow = 1, widths = c(1.4,2,1.5)) &
...
cols <- c("Other" = "#a6cee3", "Unidentified" = "#d9d9d9",
          "Acholeplasmatales" = "#969696", "Burkholderiales" = "#1f78b4",
